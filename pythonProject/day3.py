...
def get_contours(img_dilate,img_contours):
    contours,hiererchy=cv2.findContours(img_dilate,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
    for cnt in contours:
        ...
        cv2.putText(img_contours,"Points"+str(len(approx)),(x,y),(x+w,y+h),cv2.FONT_HERSHEY_COMPLEX(255,0,255), 2,0)
